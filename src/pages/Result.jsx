...
function Result() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Debug: Console mein dekhein ki kya data ja raha hai
    console.log("Sending to Backend:", JSON.stringify(result));

    try {
      // Backend ko JSON bhej rahe hain
      const response = await axios.post("http://localhost:8080/api/results/save", result, {
        headers: {
      ...
      // Form reset
      setResult({ student_id: "", subject_id: "", total_marks: "", grade: "", status: "" });
    } catch (error) {
    ...
    <div style={pageStyle}>
      <div style={cardStyle}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
          ...
          <input type="text" name="total_marks" placeholder="Total Marks" value={result.total_marks} onChange={handleChange} style={inputStyle} required />
